...
let puzzle, autoStart, shuffleIndex;
...
class Puzzle {
  ...
  */
  ...
  getContainerSize() {
    ...
    const imgElement = document.getElementById('forPuzzle').children[0];
    const dimen = document.getElementById('puzzleDiv');

    /* dimensions of container */
    this.contWidth = parseFloat(dimen.offsetWidth);
    this.contHeight = parseFloat(dimen.offsetHeight);
  }
  ...
  create() {
    ...
    */
    ...
    this.defineShapes({ coeffDecentr: 0.12, twistf: [twist0, twist1, twist2, twist3][0] });
  ...
  */
  ...
  scale() {
    ...
    const maxWidth = .985 * this.contWidth;
    const maxHeight = this.contHeight;
  ...
  optimInitial() {
    ...
    */
    ...
    const minx = -this.scalex / 2 + 50;
    const miny = -this.scaley / 2 + 20;
    const maxx = this.contWidth - 3 * this.scalex;
...
{ // scope for loadFile
  ...
  function getFile() {
    ...
    reader.addEventListener('load', () => {
      ...
      winElements != undefined && winElements.remove();
      // let peice = document.getElementById("piecesDiv")
      // peice.style.backgroundImage = `url(${reader.result})`;
      let demo = document.getElementById("forDemo");
      demo.style.backgroundImage = `url(${reader.result})`;
      puzzle.container.style.visibility = "hidden";
      setTimeout(() => {
        gsap.to(demo, { opacity: 0.4, duration: 0.5 });
        events.push({ event: "nbpieces", nbpieces: [16, 25, 36, 49, 64, 81, 100][0] })
        // constructArea.querySelector('img').style.opacity = 0.4
      }, 2500)
    });
...
function loadInitialFile() {
  puzzle.srcImage.src = "./assets/image.png";
  puzzle.container.style.visibility = "hidden";
  // let peice = document.getElementById("piecesDiv")
  // peice.style.backgroundImage = `url('./assets/image.png')`;
  let demo = document.getElementById("forDemo");
  demo.style.backgroundImage = "url('./assets/image.png')";
  setTimeout(() => {
    gsap.to(demo, { opacity: 0.4, duration: 0.5 });
    shuffleIndex = 0
    resetButton()
    events.push({ event: "nbpieces", nbpieces: [16, 25, 36, 49, 64, 81, 100][shuffleIndex] })
    // constructArea.querySelector('img').style.opacity = 0.4
  }, 2500)
}
//-----------------------------------------------------------------------------
...
function displayWin() {
  const demo = document.getElementById('forDemo')
  gsap.to(demo, { opacity: 1, duration: 0.5 })
  const peice = document.getElementById('piecesDiv')
  ...
  win.appendChild(img)
  win.appendChild(complete)
  peice.appendChild(win)
  gsap.to(img, { // Target object to rotate
    duration: 0.2, // Duration of the animation in seconds
    rotation: "-10", // Rotate 45 degrees to the left (use string for degrees)
    ease: "power1.out", // Optional ease function (adjust for desired speed)
    onComplete: () => { // Callback function after first rotation
      gsap.to(img, { // Rotate back to the right
        duration: 0.4,
        rotation: "20", // Rotate back to 0 degrees
        ease: "power1.out",
        onComplete: () => { // Callback function after first rotation
          gsap.to(img, { // Rotate back to the right
            duration: 0.2,
            rotation: "0", // Rotate back to 0 degrees
            ease: "power1.out" // Optional ease function for second rotation
          });
        } // Optional ease function for second rotation
      });
    }
  });
}
...
{ // scope for animate
  ...
  animate = function () {
    ...
    switch (state) {
      ...
      case 15:
        if (autoStart) event = { event: "nbpieces", nbpieces: 16 }; // auto start
        autoStart = false; // not twice
        if (!event) return;
        if (event.event == "nbpieces") {
          puzzle.container.style.visibility = "visible"
      ...
      case 55:  // moving piece
        ...
        switch (event.event) {
          ...
          case "leave":
            // Get the boundaries of puzzleDiv and piecesDiv
            const puzzleDiv = document.getElementById('puzzleDiv');
            const puzzleDivRect = puzzleDiv.getBoundingClientRect();

            const piecesDiv = document.getElementById('piecesDiv');
            const piecesDivRect = piecesDiv.getBoundingClientRect();

            // Get the current position of the piece being moved
            const pieceRect = moving.pp.canvas.getBoundingClientRect();

            // Calculate the center of the piece
            const pieceCenterX = pieceRect.left + pieceRect.width / 2;
            const pieceCenterY = pieceRect.top + pieceRect.height / 2;

            // Check if the center of the piece is inside puzzleDiv
            const isInsidePuzzleDiv = (
              pieceCenterX >= puzzleDivRect.left &&
              pieceCenterX <= puzzleDivRect.right &&
              pieceCenterY >= puzzleDivRect.top &&
              pieceCenterY <= puzzleDivRect.bottom
            );

            // Check if the center of the piece is inside piecesDiv
            const isInsidePiecesDiv = (
              pieceCenterX >= piecesDivRect.left &&
              pieceCenterX <= piecesDivRect.right &&
              pieceCenterY >= piecesDivRect.top &&
              pieceCenterY <= piecesDivRect.bottom
            );

            if (isInsidePuzzleDiv || isInsidePiecesDiv) {
              // The piece is within either puzzleDiv or piecesDiv, let it stay in its dropped position
            } else {
              // The piece is outside both puzzleDiv and piecesDiv, return it to its initial position
              moving.pp.moveTo(moving.ppXInit, moving.ppYInit);  // Move piece back to initial position
            }

            // Snap nearby pieces if they are close enough
            let doneSomething;
            do {
              doneSomething = false;
              for (let k = puzzle.polyPieces.length - 1; k >= 0; --k) {
                let pp = puzzle.polyPieces[k];
                if (pp == moving.pp) continue; // Don't match with itself
                if (moving.pp.ifNear(pp)) { // A match is found!
                  // Merge pieces if they are close
                  if (pp.pieces.length > moving.pp.pieces.length) {
                    pp.merge(moving.pp);
                    moving.pp = pp; // Keep track of the merged piece
                  } else {
                    moving.pp.merge(pp);
                  }
                  doneSomething = true;
            ...
            puzzle.evaluateZIndex();
            state = 50;  // Return to waiting for user interaction

            if (puzzle.polyPieces.length == 1) state = 60;  // If only one piece is left, puzzle is solved
            return;
        }
        break;
...
function checkOrientation() {
  ...
  if (orientationType.startsWith("portrait")) {
    ...
    console.log("Screen orientation cannot be determined.");
  }
}
...
// Call the function to check orientation initially
checkOrientation();
